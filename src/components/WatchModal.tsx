...
export const WatchModal: React.FC<WatchModalProps> = ({
  ...
  const currentMirror = streamData?.mirrors?.[selectedMirrorIndex] || streamData?.mirrors?.[0];
        ...
        <div className="relative w-full bg-[#06060b] aspect-video flex items-center justify-center overflow-hidden">
          {loading ? (
            ...
            <div className="p-6 text-center text-slate-300 space-y-3">
              ...
              <button
                onClick={() => {
                  setLoading(true);
                  setError(null);
                  donghuaApi.getEpisode(slug).then(setStreamData).finally(() => setLoading(false));
                }}
            ...
            <div className="text-center p-6 text-slate-400 space-y-3">
              <p className="text-sm">Video tidak dapat dimuat otomatis pada server ini.</p>
              <div className="flex flex-wrap items-center justify-center gap-2">
                {streamData?.mirrors && streamData.mirrors.length > 1 && (
                  <button
                    onClick={() => setSelectedMirrorIndex((prev) => (prev + 1) % streamData.mirrors.length)}
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-[11px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  <span>Pilih Server ({streamData.mirrors.length})</span>
                </div>
                {currentMirror?.streamUrl && (
                  <a
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {streamData.mirrors.map((mirror, idx) => (
                  <button
                    key={mirror.name || idx}
